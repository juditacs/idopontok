...
with open('input.csv', 'rb') as csvinput:
    ...
    for row in rows[1 : 1 + csoport_szam]:
        ...
        valaszok.append([float(valasz_string) for valasz_string in row[1:]])
    #print valaszok
...
def mentes():
    ...
    with open('lehetosegek.csv', 'wb') as csvfile:
        ...
        for lehetoseg in lehetosegek_sorrendben:
            sor = [kerekit(lehetoseg[0])] + lehetoseg[1:]
            csvwriter.writerow(sor)
            csvfile.flush()
    print '!!! MENTVE !!!'
...
def kerekit(szam):
    if abs(round(szam) - szam) < 0.05:
        return str(int(szam + 0.5))
    return "{:0.1f}".format(szam)
...
def feljegyez():
    ...
    for csoport_index in xrange(csoport_szam):
        ...
        lehetoseg[idopont_index] = csoportok[csoport_index] + ' (' + kerekit(valaszok[csoport_index][idopont_index]) +')'
